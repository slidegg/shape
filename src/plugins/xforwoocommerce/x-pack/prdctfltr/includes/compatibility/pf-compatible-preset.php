	...
	public static function __get_terms_from_options_for_per_page() {
		...
		for ( $i = 1; $i <= $perpage_limit; $i++ ) {
			$array[] = array(
				...
				'slug' => 'c' . ($i-1),
				'value' => $perpage_set*$i,
				'title' => $perpage_set*$i . ' ' . ( self::___u( self::$preset['wc_settings_prdctfltr_perpage_label'] ) == '' ? esc_html__( 'Products', 'xforwoocommerce' ) : self::$preset['wc_settings_prdctfltr_perpage_label'] ),
			);
		}
