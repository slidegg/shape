...
/*changed to [] check that */
	...
	function remove_list_item(e) {
		...
		if ( Object.prototype.toString.call(s) == '[object Array]' ) {
			__get_value_by_name(__get_list_item_name(e).split('[')[0]).splice($(e.target).closest('.svx-option-list-item').index(), 1);
		}
		else {
...
/*		var term = {
			id : 'c'+l,
			slug : sanitize_title('Option #'+(l+1)),
			name:'Option #'+(l+1),
		};
*/
	...
	function include_customizer_call(e,f) {
		...
		/*s.clear();
		s.clearOptions();
		s.addOption({text:'Hello',value:'hello'});
		s.refreshOptions();*/
		...
		if ( u(f.terms) ) {
			...
			if ( f.terms.length == r.length ) {
				...
				$.each( f.terms, function(n,p) {
					var h = __get_object_with_propery_id(r,o.id);
					if ( h === false ) {
						h = __get_object_with_propery_slug(r,o.slug);
					}
					if ( h === false ) {
						r.push(p);
						f.options.push( {
