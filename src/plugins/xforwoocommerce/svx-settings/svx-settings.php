...
if ( isset( $GLOBALS['svx'] ) && version_compare( $GLOBALS['svx'], '1.2.1' ) == 0 ) :
...
if ( !class_exists( 'SevenVXSettings' ) ) {
	...
	class SevenVXSettings {
		...
		?>
			<div id="svx-settings" class="svx-<?php echo esc_attr( current_filter() ); ?>"></div>
		<?php
		...
		public function add_templates() {
			...
			<script type="text/template" id="tmpl-svx-option-values-multiselect">
				<option value="{{ data.val }}"<# if ( $.inArray(data.val, data.sel) != '-1' ) { #> selected="selected"<# } else { #><# } #>>{{ data.name }}</option>
			</script>
			...
			<script type="text/template" id="tmpl-svx-option-values-select">
				<option value="{{ data.val }}"<# if ( data.sel == data.val ) { #> selected="selected"<# } else { #><# } #>>{{ data.name }}</option>
			</script>
								...
								<div id="svx-special-options">
									<span class="svx-special-option">
										...
										<select class="svx-terms-style-change" data-option="type">
											<# if ( data.type != 'ivpa_custom' || data.type != 'ivpa_attr' ) { #>
		...
		public function ajax_factory() {
			...
			switch( $_POST['svx']['type'] ) {
				...
				case 'filter' :
					wp_send_json( SevenVX()->_filter_get() );
					exit;
				break;
		...
		public function _filter_get() {
			...
			return Prdctfltr()->___get_preset( $key );
