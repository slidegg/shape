...
if(!class_exists("Ultimate_Dual_colors")){
	class Ultimate_Dual_colors{
					...
					array(
					   ...
					   "params" => array(
							...
							array(
								...
								"dependency" => Array("element" => "dual_color_layout", "value" => "inline" ),
								"group" => "Style",
							),
							array(
								"type" => "number",
								...
								"suffix" => "px",
								"description" => "",
								"dependency" => Array("element" => "dual_color_spacing", "value" => "yes" ),
								"group" => "Style",
							),
							array(
								"type" => "dropdown",
								...
								"dependency" => Array("element" => "dual_color_layout", "value" => "stack" ),
								"group" => "Style",
							),
							array(
								"type" => "ult_param_heading",
							...
							array(
								...
								"value" => "",
								"description" => "",
								"dependency" => Array("element" => "dual_ba_border", "not_empty" => true),
								"group" => "Style"
							),
							array(
								"type" => "number",
							...
							array(
								...
								"value" => "",
								"description" => "",
								"dependency" => Array("element" => "dual_high_border", "not_empty" => true),
								"group" => "Style"
							),
							array(
								"type" => "number",
		...
		// Shortcode function
		function ultimate_dual_colors_module_shortcode($atts, $content = null){
			$dual_before_txt = $dual_high_link = $dual_after_link = $dual_high_txt = $dual_after_txt = $dual_before_link = $dual_color_align = $dual_align = $heading_tag = $dual_ba_color = $dual_ba_inline = $dual_high_color = $dual_high_inline = $dual_color_spacing = $dual_color_width = $dual_inlinel_width = $dual_inliner_width = $href = $target = $link_title = $rel = $url = $blink_prefix = $blink_sufix = $hlink_prefix = $hlink_sufix = $alink_prefix = $alink_sufix = $dual_color_font_family = $dual_color_font_style = $dcfont_family = $dchfont_family= $main_dual_color_style_inline = $main_dual_high_style_inline = $dual_color_font_size = $dual_color_line_height = $main_dual_color_responsive = $main_dual_high_responsive = $dual_color_high_font_family = $dual_color_high_font_style = $dual_color_high_font_size = $dual_color_high_line_height = $dual_ba_back_inline = $dual_ba_back_color = $dual_high_back_color = $dual_high_back_inline = $dual_inline_width = $dual_ba_transform = $dual_high_transform = $dual_ba_trans_inline = $dual_high_trans_inline = $dual_ba_letter_space = $dual_high_letter_space = $dual_ba_ls_inline = $dual_high_ls_inline = $dual_ba_padding = $dual_high_padding = $dual_ba_padding_inline = $dual_high_padding_inline = $dual_ba_border = $dual_ba_bcolor = $dual_ba_bstyle = $dual_ba_bradius = $dual_high_border = $dual_high_bcolor = $dual_high_bstyle = $dual_high_bradius = $dual_ba_border_inline = $dual_high_border_inline = $dual_design_style_css = $dual_color_layout = $dual_color_stack = $el_class = $output = '';
			...
			$dual_design_style_css = apply_filters( VC_SHORTCODE_CUSTOM_CSS_FILTER_TAG, vc_shortcode_custom_css_class( $css_dual_color_design, ' ' ), "ultimate_ribbons", $atts );
			 ...
			 $dual_design_style_css = esc_attr( $dual_design_style_css );
				...
				if($dual_before_txt == '')
					$dual_before_txt = 'I Love';

				if($dual_high_txt == '')
					$dual_high_txt = 'this website';
				//Dual-Color Alignment 
				if($dual_color_align != '')
				{
					$dual_align = 'text-align:'.$dual_color_align.';';
				}
				...
				if($dual_ba_color != '')
				{
					$dual_ba_inline = 'color:'.$dual_ba_color.';';
				}
				//Highlighted text color for dual-color
				if($dual_high_color != '')
				{
					$dual_high_inline = 'color:'.$dual_high_color.';';
				}
				//Before & after Background color for dual-color
				if($dual_ba_back_color != '')
				{
					$dual_ba_back_inline = 'background-color:'.$dual_ba_back_color.';';
				}
				//Highlighted background color for dual-color
				if($dual_high_back_color != '')
				{
					$dual_high_back_inline = 'background-color:'.$dual_high_back_color.';';
				}

				//Text -Transform Property for Before & After heading.
				if($dual_ba_transform != '')
				{
					$dual_ba_trans_inline = 'text-transform: '.$dual_ba_transform.';';
				}

				//Text -Transform Property for Highlighted heading.
				if($dual_high_transform != '')
				{
					$dual_high_trans_inline = 'text-transform: '.$dual_high_transform.';';
				}

				//Letter spacing for Before & After heading.
				if($dual_ba_letter_space !== '')
					$dual_ba_ls_inline = 'letter-spacing:'.$dual_ba_letter_space.'px;';

				//Letter spacing for Highlighted heading.
				if($dual_high_letter_space !== '')
					$dual_high_ls_inline = 'letter-spacing:'.$dual_high_letter_space.'px;';

				/* Before and after padding */
				if( $dual_ba_padding != '' ){
					$dual_ba_padding_inline = $dual_ba_padding;
				}

				/* Highlighted padding */
				if( $dual_high_padding != '' ){
					$dual_high_padding_inline = $dual_high_padding;
				}
				// Border style for Before & after headings.
				if($dual_ba_border !== ''){
				$dual_ba_border_inline .= 'border-radius:'.$dual_ba_bradius.'px;';
				$dual_ba_border_inline .= 'border-width:'.$dual_ba_bstyle.'px;';
				$dual_ba_border_inline .= 'border-color:'.$dual_ba_bcolor.';';
				$dual_ba_border_inline .= 'border-style:'.$dual_ba_border.';';
				} else {
				...
				if($dual_high_border !== ''){
				$dual_high_border_inline .= 'border-radius:'.$dual_high_bradius.'px;';
				$dual_high_border_inline .= 'border-width:'.$dual_high_bstyle.'px;';
				$dual_high_border_inline .= 'border-color:'.$dual_high_bcolor.';';
				$dual_high_border_inline .= 'border-style:'.$dual_high_border.';';
				} else {
					$dual_high_border_inline .= 'border:none;';
				}

				if($dual_color_spacing == 'yes')
				{
					if($dual_color_width != '')
						{
							$dual_inline_width = 'margin-left:'.$dual_color_width.'px;';
							$dual_inline_width .= 'margin-right: '.$dual_color_width.'px;';
						}
				}
			// Link for the Before Text
			if($dual_before_link !== ''){
				$href = vc_build_link($dual_before_link);
				if($href['url'] !== ""){
			...
			if($dual_high_link !== ''){
				$href = vc_build_link($dual_high_link);
				if($href['url'] !== ""){
					$url 			= ( isset( $href['url'] ) && $href['url'] !== '' ) ? $href['url']  : '';
					$target 		= ( isset( $href['target'] ) && $href['target'] !== '' ) ? esc_attr( trim( $href['target'] ) ) : '';
					$link_title 	= ( isset( $href['title'] ) && $href['title'] !== '' ) ? esc_attr($href['title']) : '';
			...
			if($dual_after_link !== ''){
				$href = vc_build_link($dual_after_link);
				if($href['url'] !== ""){
					$url 			= ( isset( $href['url'] ) && $href['url'] !== '' ) ? $href['url']  : '';
					$target 		= ( isset( $href['target'] ) && $href['target'] !== '' ) ? esc_attr( trim( $href['target'] ) ) : '';
					$link_title 	= ( isset( $href['title'] ) && $href['title'] !== '' ) ? esc_attr($href['title']) : '';
			...
			if($dual_color_font_family != '')
			{
				$dcfont_family = get_ultimate_font_family($dual_color_font_family);
				if($dcfont_family)
					$main_dual_color_style_inline .= 'font-family:\''.$dcfont_family.'\';';
			}
			/* ----  Before and after font style---- */
			$main_dual_color_style_inline .= get_ultimate_font_style($dual_color_font_style);

			// FIX: Before and after font size before implementing responsive param
			if(is_numeric($dual_color_font_size)) 	{ 	$dual_color_font_size = 'desktop:'.$dual_color_font_size.'px;';		}
			if(is_numeric($dual_color_line_height)) 	{ 	$dual_color_line_height = 'desktop:'.$dual_color_line_height.'px;';		}
			// Before and after responsive {main} video styles
		  	$args = array(
		  		'target'		=>	'.ult-dual-color.'.$uid. ' .ult-dual-heading-text',
		  		'media_sizes' 	=> array(
					'font-size' 	=> $dual_color_font_size,
					'line-height' 	=> $dual_color_line_height,
				),
		  	);
			$main_dual_color_responsive = get_ultimate_vc_responsive_media_css($args);

			/* ---- Highlighted heading styles ---- */
			if($dual_color_high_font_family != '')
			{
				$dchfont_family = get_ultimate_font_family($dual_color_high_font_family);
				if($dchfont_family)
					$main_dual_high_style_inline .= 'font-family:\''.$dchfont_family.'\';';
			}
			/* ----  Highlighted font style---- */
			$main_dual_high_style_inline .= get_ultimate_font_style($dual_color_high_font_style);
			// FIX: Highlighted font size before implementing responsive param
			if(is_numeric($dual_color_high_font_size)) 	{ 	$dual_color_high_font_size = 'desktop:'.$dual_color_high_font_size.'px;';		}
			if(is_numeric($dual_color_high_line_height)) 	{ 	$dual_color_high_line_height = 'desktop:'.$dual_color_high_line_height.'px;';		}
			// Highlighted responsive {main} video styles
		  	$args = array(
		  		'target'		=>	'.ult-dual-color.'.$uid. ' .ult-highlight-text',
		  		'media_sizes' 	=> array(
					'font-size' 	=> $dual_color_high_font_size,
					'line-height' 	=> $dual_color_high_line_height,
				),
		  	);
			$main_dual_high_responsive = get_ultimate_vc_responsive_media_css($args);

			$output = '<div id="'.esc_attr( $uid ).'" class="ult-dual-color '.esc_attr( $is_vc_49_plus ).' ult-dual-color-responsive-'.esc_attr($dual_color_stack).' '.esc_attr( $uid ).' '.esc_attr( $el_class ).' '.esc_attr( $dual_design_style_css ).'">';
